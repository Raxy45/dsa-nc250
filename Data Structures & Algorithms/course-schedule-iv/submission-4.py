class Solution:
    def checkIfPrerequisite(self, numCourses: int, prerequisites: List[List[int]], queries: List[List[int]]) -> List[bool]:
        complete_prereq = [set([]) for _ in range(numCourses)]
        graph = [[] for _ in range(numCourses)]

        # construct graph
        for pre_req, course in prerequisites:
            graph[course].append(pre_req)

        def dfs(course):
            if len(complete_prereq[course]) > 0:
                return complete_prereq[course]
            if graph[course] == []:
                return set([])

            for pre_req in graph[course]:
                complete_prereq[course].add(pre_req)
                complete_prereq[course].update(dfs(pre_req))
            
            return complete_prereq[course]

        # contruct complete pre-reqs
        visit = set()
        for i in range(numCourses):
            dfs(i)

        ans = [False] * len(queries)

        # print(f'{graph = }')
        # print(f'{complete_prereq = }')

        # print('queries')
        for i, query in enumerate(queries):
            # print(query[0], query[1])
            # print(complete_prereq[query[1]])
            if query[0] in complete_prereq[query[1]]: ans[i] = True
        return ans