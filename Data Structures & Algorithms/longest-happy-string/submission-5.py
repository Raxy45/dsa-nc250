    ...
    def longestDiverseString(self, a: int, b: int, c: int) -> str:
        ...
        while hp:
            ...
            if len(ans) > 1 and ans[-1] == ans[-2] == char:
                ...
                # You can use the popped out char, then decrease its freq by 1, add to ans
                ans += char
                freq = freq + 1
            if freq:
                # anyway even if you used the char or not, if freq of popped out char is > 0
                # you will have to add it back to heap
                heapq.heappush(hp, (freq, char))
