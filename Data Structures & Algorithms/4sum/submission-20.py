    ...
    def fourSum(self, nums: List[int], target: int) -> List[List[int]]:
        ...
        for i in range(len(nums)):
            ...
            for j in range(i + 1, len(nums)):
                ...
                if j>(i+1): # We want to check duplicity of j th index, when it the j at
                # least becomes j+1. as for very first j after i. j-1 is equal to i and is not
                # even in the range
                
                # we've something like [1,1,2,3,4]  We skip this because when j
