class Solution:
    def maxTurbulenceSize(self, arr: List[int]) -> int:
        ...
        prevSign = 0      # Previous comparison sign
        curr = 1          # Length of turbulent subarray ending at current index
        ans = 1

        for i in range(1, n):

            # Compute current comparison sign
            if arr[i] > arr[i - 1]:
                currSign = 1
            elif arr[i] < arr[i - 1]:
                currSign = -1
            ...
            if currSign == 0:
                # Equal elements -> cannot form turbulence
                curr = 1

            elif currSign != prevSign:
                # Sign alternates -> extend current turbulent subarray
                curr += 1

            else:
                # Same sign as previous
                # Current pair starts a new turbulent subarray
                curr = 2
        ...
        return ans
    def maxTurbulenceSizeON(self, arr: List[int]) -> int:
        n = len(arr)
        if n == 1:
            return 1
