# Definition for a binary tree node.
# class TreeNode:
#     def __init__(self, val=0, left=None, right=None):
#         self.val = val
#         self.left = left
#         self.right = right
class Solution:
    def inorderTraversalMorris(self, root):
        ans, curr = [], root
        while curr:
            # print('inside')
            if curr.left == None:
                ans.append(curr.val)
                curr = curr.right
            else:
                left_child = curr.left
                while left_child.right and left_child.right != curr:
                    left_child = left_child.right
                
                if left_child.right == None:
                    # We are at end, we need to bind this end to curr
                    left_child.right = curr
                    curr = curr.left
                elif left_child.right == curr:
                    # This means, we've have already traversed the left subtree
                    left_child.right = None # removing the link
                    ans.append(curr.val)
                    curr = curr.right
        return ans



    def inorderTraversal(self, root):
        stk = []
        curr = root
        lst = []
        while stk or curr:
            while curr:
                stk.append(curr)
                curr = curr.left
            node = stk.pop()
            if node:
                print(node.val, 'hhh')
                lst.append(node.val)
                curr = node.right
        return lst
    def inorderTraversalRec(self, root):
        def dfs(root):
            if root:
                dfs(root.left)
                print(root.val)
                lst.append(root.val)
                dfs(root.right)
        lst = []
        dfs(root)
        return lst



    def inorderTraversalStk(self, root):
        ans = []
        curr = root
        while curr:
            l_c = curr.left
            if not l_c:
                ans.append(curr.val)
                curr = curr.right
            else:
                while l_c.right and l_c.right != curr:
                    l_c = l_c.right
                
                if l_c.right == None:
                    # we have connection to make
                    l_c.right = curr
                    curr = curr.left
                else:
                    l_c.right = None
                    ans.append(curr.val)
                    curr = curr.right
        return ans

    def inorderTraversalStk(self, root):
        stk = []
        curr = root
        ans = []
        while curr or stk:
            while curr:
                stk.append(curr)
                curr = curr.left
            
            node = stk.pop()
            ans.append(node.val)
            curr = node.right
        return ans
    def inorderTraversalRec(self, root: Optional[TreeNode]) -> List[int]:
        def dfs(node):
            if not node: return
            dfs(node.left)
            ans.append(node.val)
            dfs(node.right)
        
        ans = []
        dfs(root)
        return ans