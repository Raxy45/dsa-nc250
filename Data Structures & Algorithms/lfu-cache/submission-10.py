...
class LFUCache:
    ...
    def get(self, key: int) -> int:
        if key not in self.global_mp:
            print('key', key, 'does not exist in global_mp')
            return -1
    ...
    def useCounter(self, node):
        ...
        if current_lru.isEmpty():
            print('deleting entire lru as it is empty, for freq', current_freq)
    ...
    def put(self, key: int, value: int) -> None:
        print('inserting key, value', key, value, self.global_mp)
        ...
        if self.k == 0:
            ...
            lfu_lru.remove_lru()
            print('deleting ', popped_node.key, 'for ', key, value)
            del self.global_mp[popped_node.key]
            if lfu_lru.isEmpty():
                del self.lru_map[self.lowest_freq]
                self.lowest_freq += 1
            self.k += 1
        else:
            print('capacity is not full for',key, value)
