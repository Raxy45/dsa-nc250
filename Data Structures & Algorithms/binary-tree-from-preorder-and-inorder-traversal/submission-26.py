...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        # I use preorder to pick roots and inorder index ranges to recursively build left and right subtrees in O(n) time.
        # TC: O(n) -> Each node created once
        # SC: O(h) -> height of tree -> recursion stk
        #     log n -> balanced, n -> skewed
        # Aux: O(h)
        # Total: O(n)
        self.pre_index = 0  # pointer in preorder
        ...
        def construct(left, right):
            ...
            current_root.left = construct(left, root_index_in_inorder-1)
